...
std::vector<std::string> IPDB = {"172.16.117.60, 172.16.117.59"};
...
// Define the XML template for large number plates
#define XML_GateL "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"                                                                             \
                  "<sdk guid=\"b04f672504da67f19d48edb3b173c15f\">\n"                                                                        \
                  "  <in method=\"AddProgram\">\n"                                                                                           \
                  "    <screen timeStamps=\"1744104321754\">\n"                                                                              \
                  "      <program type=\"normal\" id=\"0\" guid=\"b41bbdb7-1b15-4360-9127-2f25dc3ec2b9\" name=\"\">\n"                       \
                  ...
                  "            <text singleLine=\"true\">\n"                                                                                 \
                  "              <style align=\"center\" />\n"                                                                               \
                  "              <font name=\"Arial\" size=\"24\" color=\"#ff0000\" bold=\"true\" italic=\"false\" underline=\"false\" />\n" \
                  "              <effect duration=\"100\" in=\"0\" inSpeed=\"4\" outSpeed=\"4\" out=\"20\"/>\n"                              \
                  "              <string>%s</string>\n"                                                                                      \
                  "            </text>\n"                                                                                                    \
                  "          </resources>\n"                                                                                                 \
...
void Display(HSession *session)
{
    ...
    if (Device == "Gate In")
    {
        ...
        else if (Gate_TL.size() > 9)
        {
            xmlTemplate = XML_GateL; // Macro to define the XML template
            snprintf(finalXml, sizeof(finalXml), xmlTemplate, Gate_IN_Greeting, Gate_TL.c_str());
        }
    }
    else if (Device == "Gate Out")
    {
       if (Gate_TL.size() <= 5)
        ...
        {
            xmlTemplate = XML_GateL; // Macro to define the XML template
            snprintf(finalXml, sizeof(finalXml), xmlTemplate, Gate_OUT_Greeting, Gate_TL.c_str());
        }
    }
    else
    {
